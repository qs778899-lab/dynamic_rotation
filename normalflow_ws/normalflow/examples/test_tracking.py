...
def test_tracking():
    ...
    G_ref, H_ref, C_ref = recon.get_surface_info(tactile_images[0], ppmm) #radients (G), height map (H), and contact mask (C).
    C_ref = erode_contact_mask(C_ref) #去掉毛边或噪声
    N_ref = gxy2normal(G_ref) #梯度转法线图
    ...
    M_ref = cv2.moments(max(contours_ref, key=cv2.contourArea)) # 计算最大轮廓的中心（质心）
    cx_ref, cy_ref = int(M_ref["m10"] / M_ref["m00"]), int(M_ref["m01"] / M_ref["m00"]) #求出轮廓中心坐标

    # Track the target frames with regards to the reference frame
    tracked_tactile_images = []
    curr_T_ref_init = np.eye(4) #当前帧和参考帧重合
