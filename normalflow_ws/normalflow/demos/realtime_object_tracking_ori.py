...
def realtime_object_tracking():
    ...
    while is_running:
        ...
        if contact_area < 500:
            ...
            G_start, H_start, C_start = recon.get_surface_info(image_start, ppmm) #radients (G), height map (H), and contact mask (C).
            C_start = erode_contact_mask(C_start) # 去掉毛边或噪声

            if np.sum(C_start) < 500:
                print("C_start too small, skip.")
                continue
            
            N_start = gxy2normal(G_start) # 梯度转法线图
            ...
            if not contours_start:
                print("Warning: No contours found in contours_start!")
                continue
            M_start = cv2.moments(max(contours_start, key=cv2.contourArea)) # 计算最大轮廓的中心（质心）
            ...
            # wwj 设定第一帧相对于世界坐标系的位姿
            theta_z = np.radians(0)  # Z轴旋转30度
            theta_x = np.radians(0)  # X轴旋转90度
            ...
            # 变换矩阵 ref_T_world
            ref_T_world = np.eye(4)
            ref_T_world[:3, :3] = R
            ref_T_world[:3, 3] = t.squeeze()
            ...
            while is_tracking:
                ...
                if N_prev is not None:
                    try:
                        ...
                        print("Reset failed: prev → curr overlap also insufficient. Skipping frame.")
                        is_tracking = False  # 或 continue，或跳帧等
                        continue
                    if not is_reset:
                ...
                # 将中心和方向向量从 ref 坐标变换到世界坐标
                center_3d_world = np.dot(ref_T_world[:3, :3], center_3d_ref) + ref_T_world[:3, 3]
                unit_vectors_world = np.dot(ref_T_world[:3, :3], unit_vectors_3d_ref.T).T

                # 将中心点和坐标轴方向重新投影到图像平面
                remapped_cx_ref = center_3d_world[0] * 1000 / ppmm + imgw / 2 - 0.5
                remapped_cy_ref = center_3d_world[1] * 1000 / ppmm + imgh / 2 - 0.5
                remapped_center_ref = np.array([remapped_cx_ref, remapped_cy_ref]).astype(np.int32)

                remapped_unit_vectors_ref = (
                    unit_vectors_world[:, :2] * 1000 / ppmm  # 只取X/Y投影方向
                )

                # 绘图
                annotate_coordinate_system(image_l, remapped_center_ref, remapped_unit_vectors_ref)
                ...
                curr_T_start = curr_T_ref @ np.linalg.inv(start_T_ref) #当前帧相对于第一帧 = 当前帧相对于参考帧 @ 每次参考帧更新后，相对于最初参考帧的累计变换
                curr_T_world = ref_T_world @ curr_T_start #wwj 换乘到世界坐标系

                # curr_T_start--> curr_T_world
                remapped_center_3d_start = (
                    np.dot(curr_T_world[:3, :3], center_3d_start) + curr_T_world[:3, 3]
                )
                remapped_cx_start = (
                ...
                remapped_unit_vectors_start = (
                    np.dot(curr_T_world[:3, :3], unit_vectors_3d_start.T).T
                    * 1000
                    / ppmm
                )[:, :2]
